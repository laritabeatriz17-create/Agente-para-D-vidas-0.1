...
# Tenta primeiro pegar de st.secrets (usado no Streamlit Community Cloud),
# e se não existir, tenta pegar da variável de ambiente GROQ_API_KEY.
api_key = st.secrets.get("GROQ_API_KEY", os.environ.get("GROQ_API_KEY"))

if not api_key:
    st.error(
        "Chave da API da Groq não encontrada.\n\n"
        "Configure de uma das formas abaixo:\n"
        "1. Crie o arquivo `.streamlit/secrets.toml` com:\n"
        '   GROQ_API_KEY = "sua_chave_aqui"\n\n'
        "2. Ou defina a variável de ambiente antes de rodar:\n"
        ...
        try:
            with st.spinner("Pensando..."):
                resposta = client.chat.completions.create(
                    model="openai/gpt-oss-120b",  # llama-3.3-70b-versatile foi descontinuado pela Groq
                    temperature=0.7,
                    messages=mensagens,
                )
